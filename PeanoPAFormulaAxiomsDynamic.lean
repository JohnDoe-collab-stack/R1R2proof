...
/-- Temporal state: the currently active formula-component regime. -/
abbrev TemporalState := PAFormulaComponentActiveStep

/-- Horizon index for temporal residuals. -/
abbrev TemporalHorizon := Nat

/-- Dynamic time index. -/
abbrev TemporalTime := Nat

/-- Windows are local views centered on an active component regime. -/
abbrev TemporalWindow := PAFormulaComponentActiveStep

/-- Canonical induction formula used in the temporal cycle. -/
def temporalPhi : Formula :=
  PeanoPAFormulaAxioms.phi0

/--
External temporal transition.

The state cycles through addition, multiplication, and the induction instance
at `temporalPhi`.  Any other induction parameter returns to addition at the
next temporal step.
-/
def nextTemporalState : TemporalState → TemporalState
  | .addition => .multiplication
  | .multiplication => .induction temporalPhi
  | .induction _phi => .addition

/-- Concrete temporal trajectory generated by `nextTemporalState`. -/
def temporalStateAt : TemporalTime → TemporalState
  | Nat.zero => .addition
  | Nat.succ k => nextTemporalState (temporalStateAt k)

/-- A local window contains exactly its selected temporal state. -/
def temporalInWindow (W : TemporalWindow) (x : TemporalState) : Prop :=
  W = x

/-- Window inclusion for exact local windows. -/
def temporalWindowLe (W W' : TemporalWindow) : Prop :=
  W = W'

/-- The raw dynamic residual attached to a temporal state and exact window. -/
def rawResidualAt
    (_r : TemporalHorizon) (W : TemporalWindow) (x : TemporalState) : Prop :=
  temporalInWindow W x ∧
    DynamicResidualNonempty_R2
      obs_PA_formula_component
      target_PA_formula_component_active
      I_PA_formula_component x

/-- Raw residual profile: unmediated R1 residuals transport through time. -/
def rawTemporalResidualProfile :
    DynamicResidualProfile
      TemporalState TemporalHorizon TemporalTime TemporalWindow :=
  { stepState := nextTemporalState
    nextHorizon := Nat.succ
    nextTime := Nat.succ
    InWindow := temporalInWindow
    WindowLe := temporalWindowLe
    ResidualAt := rawResidualAt
    restrict := by
      intro r W W' x hLe hIn hResidual
      exact ⟨hIn, hResidual.2⟩
    persist := by
      intro r W x hResidual
      exact hResidual
    transport := by
      intro r W x _hResidual
      exact
        ⟨nextTemporalState x,
          rfl,
          ⟨rfl,
...
/-- Raw temporal residual coordinate. -/
def rawTemporalResidualCoordinate :
    DynamicResidualCoordinate rawTemporalResidualProfile :=
  { rhoAt := fun _r _W => 1
    positive_of_residual := by
      intro r W x hIn hResidual
...
/-- The concrete raw temporal trajectory is a stable residual section. -/
def rawStableResidualSection :
    StableResidualSection rawTemporalResidualProfile :=
  { windowAt := fun _r k => temporalStateAt k
    stateAt := temporalStateAt
    stateInWindow := by
      intro r k
      rfl
    residualAt := by
      ...
      exact
        ⟨rfl,
          dynamicResidualNonempty_PA_formula_component_active
            (temporalStateAt k)⟩
    transitionCompatible := by
      intro k
      rfl }

/-- Raw residual witnesses transport along the concrete temporal trajectory. -/
theorem rawStableResidualSection_transports
    (r : TemporalHorizon) (k : TemporalTime) :
    ∃ W' : TemporalWindow,
      rawTemporalResidualProfile.InWindow W'
        (rawStableResidualSection.stateAt
          (rawTemporalResidualProfile.nextTime k)) ∧
        rawTemporalResidualProfile.ResidualAt r W'
          (rawStableResidualSection.stateAt
            (rawTemporalResidualProfile.nextTime k)) :=
  stableResidualSection_transport rawStableResidualSection r k

/-- Every point of the raw stable section has positive residual coordinate. -/
theorem rawStableResidualCoordinate_positive
    (r : TemporalHorizon) (k : TemporalTime) :
    0 <
      rawTemporalResidualCoordinate.rhoAt r
        (rawStableResidualSection.windowAt r k) :=
  rhoAt_pos_of_stableResidualSection
    rawTemporalResidualCoordinate rawStableResidualSection r k

/--
Mediated residual at a temporal state.

Unlike `rawResidualAt`, this asks for an actual residual that remains after the
step-indexed mediator has been added.
-/
def mediatedResidualAt
    (_r : TemporalHorizon) (W : TemporalWindow) (x : TemporalState) : Prop :=
  temporalInWindow W x ∧
    ∃ a b : PAFormulaComponent,
      MediatedResidual
        obs_PA_formula_component
        (target_PA_formula_component_active.targetAt x)
        I_PA_formula_component
        (M_PA_formula_component_active x)
        a b

/-- No mediated residual exists at any temporal point. -/
theorem no_mediatedResidualAt
    (r : TemporalHorizon) (W : TemporalWindow) (x : TemporalState) :
    ¬ mediatedResidualAt r W x := by
  intro hResidualAt
  rcases hResidualAt with ⟨_hIn, a, b, hResidual⟩
  exact
    dynamicMediatedResidualEmpty_M_PA_formula_component_active
      x a b hResidual

/-- Mediated temporal profile: transport is vacuous because mediation closes residuals. -/
def mediatedTemporalResidualProfile :
    DynamicResidualProfile
      TemporalState TemporalHorizon TemporalTime TemporalWindow :=
  { stepState := nextTemporalState
    nextHorizon := Nat.succ
    ...
    InWindow := temporalInWindow
    WindowLe := temporalWindowLe
    ResidualAt := mediatedResidualAt
    restrict := by
      intro r W W' x hLe hIn hResidual
      exact False.elim (no_mediatedResidualAt r W' x hResidual)
    persist := by
      intro r W x hResidual
      exact False.elim (no_mediatedResidualAt (Nat.succ r) W x hResidual)
    transport := by
      intro r W x hResidual
      exact False.elim (no_mediatedResidualAt (Nat.succ r) W x hResidual) }

/-- Mediated temporal residual coordinate is identically zero. -/
def mediatedTemporalResidualCoordinate :
    DynamicResidualCoordinate mediatedTemporalResidualProfile :=
  { rhoAt := fun _r _W => 0
    positive_of_residual := by
      intro r W x hIn hResidual
      exact False.elim (no_mediatedResidualAt r W x hResidual)
...
/-- There is no stable mediated residual section. -/
theorem noStableMediatedResidualSection :
    NoStableResidualSection mediatedTemporalResidualProfile := by
  intro stableSection
  exact
    no_mediatedResidualAt 0
      (stableSection.windowAt 0 0)
      (stableSection.stateAt 0)
      (stableSection.residualAt 0 0)

/-- Global mediated temporal closure: every temporal state has mediated closure. -/
def GlobalMediatedTemporalClosure : Prop :=
  ∀ step : TemporalState,
    DynamicMediatedResidualEmpty
      obs_PA_formula_component
      target_PA_formula_component_active
...
/-- Direct global mediated closure from the uniform component certificate. -/
theorem globalMediatedTemporalClosure_direct :
    GlobalMediatedTemporalClosure := by
  intro step
  exact
    dynamicMediatedResidualEmpty_M_PA_formula_component_active
      step
...
/--
Closure bridge for the mediated temporal profile.

Here global non-closure is exactly the existence of a stable mediated residual
section.
-/
def mediatedTemporalClosureBridge :
    DynamicResidualClosureBridge mediatedTemporalResidualProfile :=
  { GlobalClosure := GlobalMediatedTemporalClosure
    GlobalNonClosure := StableResidualSection mediatedTemporalResidualProfile
    stableSectionOfNonClosure := fun h => h
    closureOfNoGlobalNonClosure := by
      intro _hNoGlobalNonClosure
      exact globalMediatedTemporalClosure_direct }

/-- Absence of stable mediated residual sections implies global mediated closure. -/
theorem globalMediatedTemporalClosure_of_noStable :
    mediatedTemporalClosureBridge.GlobalClosure :=
  globalClosure_of_noStableResidualSection
    mediatedTemporalClosureBridge
    noStableMediatedResidualSection

/-- Every temporal state has exact proper mediated R2 dimension `2`. -/
theorem temporalExactProperMediatedR2Dimension_two
    (step : TemporalState) :
    DynamicExactProperMediatedR2Dimension
      obs_PA_formula_component
      target_PA_formula_component_active
      I_PA_formula_component step 2 :=
  dynamicExactProperMediatedR2Dimension_two_PA_formula_component_active
    step

/-- Direct R1 closure fails at every temporal state before mediation. -/
theorem temporal_not_dynamicClosed_R2
    (step : TemporalState) :
    ¬ DynamicClosed_R2
      obs_PA_formula_component
      target_PA_formula_component_active
      I_PA_formula_component step :=
  not_dynamicClosed_R2_PA_formula_component_active
    step
...
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.nextTemporalState
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.rawTemporalResidualProfile
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.rawStableResidualSection
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.rawStableResidualSection_transports
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.mediatedTemporalResidualProfile
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.noStableMediatedResidualSection
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.mediatedTemporalClosureBridge
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.globalMediatedTemporalClosure_of_noStable
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.temporalExactProperMediatedR2Dimension_two
#print axioms LocalSemanticClosure.PeanoPAFormulaAxiomsDynamic.temporal_not_dynamicClosed_R2
/- AXIOM_AUDIT_END -/
