...
/-- Schema roles used by the formula-component layer. -/
...
/-- Formula-component roles available to later R1/R2 certificates. -/
...
/-- A formula component family, for later R1/R2 certificates. -/
...
For every `n >= 2`, the following section builds actual ZFC formula
components from Separation schema instances.
...
/-- ZFC formula component with a finite R2 parameter coordinate. -/
structure ZFCFiniteParameterComponent (n : Nat) where
  component : ZFCFormulaComponent
  coordinate : Fin n
...
def zfcFiniteParameterComponent {n : Nat}
  ...
  { component := zfcParameterFormulaComponent i.val
    coordinate := i }
...
`axiom` constructor and, in the same carrier, the finite Separation parameter
components used to witness exact dimension `n`.
-/
...
formula components used for the exact dimension theorem.
-/
...
def sigma_ZFC_all {n : Nat}
  ...
  match s with
  | ZFCAllAxiomFiniteState.axiom i _ => i
...
/--
For every `n >= 2`, the full ZFC axiom carrier with finite Separation
parameter components has exact proper mediated R2 dimension `n`.
-/
